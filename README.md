...
| Driver (`lib/bp_monitor.py`) | ตอน `connect()` จะถาม `STATUS` ก่อน ถ้า firmware ตอบว่า `MEASURING`/`WAIT_SHUTDOWN`/`TRIGGER` แปลว่ารอบก่อนยังไม่จบ จะ**ไม่ RESET ทับ** แต่แจ้งผู้ใช้ทันที ถ้าตอบว่าว่าง (`IDLE`/`DONE`) ก็ข้าม RESET ไปเลย และถ้าไม่ตอบ (firmware รุ่นเก่า) จึงค่อย fallback ไปส่ง `RESET` แล้วรอ `READY` แบบเดิม |
...
| Protocol | `BP_ERROR:<n>` ส่งรหัสข้อผิดพลาดของโมดูลต่อมาให้แอปด้วย (เดิม firmware อ่านรหัสแล้วทิ้ง เหลือแค่ `BP_ERROR` เปล่า ๆ) รหัสนี้คือสิ่งเดียวที่แยก "แขนขยับ" ออกจาก "ผ้าพันหลวม" ออกจาก "ลมรั่ว" ได้ — ความหมายของแต่ละรหัสยังไม่มีตารางจากผู้ผลิต จึงยังไม่เดา รหัสที่ยังไม่รู้ความหมายจะแสดงเป็นตัวเลขให้ผู้ใช้แจ้งกลับมา และเก็บความหมายที่ยืนยันแล้วไว้ใน `_BP_DEVICE_CODE_MESSAGES` (ใช้ `sensor_tests/bp_error_codes.py` ทดลองทีละสถานการณ์เพื่อสร้างตาราง) |
| Protocol | `NO_RESULT` แจ้งรอบที่วัดครบแต่แพ็กเก็ตผลหาย เดิม firmware เงียบสนิท แอปจึงรอจนหมด timeout แล้วบอกว่า "ไม่ตอบสนอง" ทั้งที่ผู้ป่วยโดนรัดแขนครบรอบไปแล้ว |

**เสริมที่ฝั่ง UI:** ปุ่มวัดความดันมี cooldown กันผู้ใช้กดรัว ๆ ตอนที่โมดูลยังไม่พร้อม — **70 วินาทีหลังวัดสำเร็จ** (ค่าอ่านถึงจอตอนที่โมดูลเพิ่งเริ่มปิดตัว ซึ่งยังต้องใช้อีก ~60 วินาที การตั้ง 60 พอดีทำให้รอบถัดไปไปตกคาบเส้น บางครั้งวัดได้บางครั้งไม่ได้) และ **60 วินาทีหลังเจอ device error หรือรอบที่จบโดยไม่ได้ค่า**
