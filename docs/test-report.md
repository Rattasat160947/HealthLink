...
- **ปรับปรุงล่าสุด:** 2 กันยายน 2569 (2026-09-02) — เพิ่มการวินิจฉัยสาเหตุที่สัญญาณ SpO2 ใช้ไม่ได้ (แสงอิ่มตัว / นิ้ววางไม่ตรงกลาง / ไม่มีจังหวะชีพจร / สัญญาณขาดช่วง) แทนข้อความรวม "สัญญาณอ่อนเกินไป"
...
|---|---|
| จำนวนกรณีทดสอบ (test cases) ทั้งหมด | **261** |
| ผ่าน | **261 (100%)** |
| ไม่ผ่าน | 0 |
| ความครอบคลุมของโค้ด (code coverage) โมดูลตรรกะ | **71%** (1,616 statements, พลาด 468) |
...
> รอบล่าสุดนับได้ **261 กรณี** ผ่านทั้งหมด (เพิ่มการวินิจฉัยสาเหตุของสัญญาณ SpO2 ที่คำนวณค่าไม่ได้
...
| **SpO2 (MAX30102): การหาค่านิ่ง + การวินิจฉัยสาเหตุที่สัญญาณใช้ไม่ได้** | `test_spo2_stability.py` | **34** |
| **SpO2: retry / timeout / ข้อความบอกสาเหตุ ระดับ provider** | `test_providers_spo2.py` | **21** |
...
| `lib/spo2_max30102/spo2_monitor.py` | 212 | 29 | **86%** |
| `lib/ups.py` | 82 | 15 | **82%** |
...
# แสดงรายชื่อกรณีทดสอบทั้ง 261 รายการ
