...
- **ปรับปรุงล่าสุด:** 2 กันยายน 2569 (2026-09-02) — เพิ่มการวินิจฉัยสาเหตุที่สัญญาณ SpO2 ใช้ไม่ได้ (แสงอิ่มตัว / นิ้ววางไม่ตรงกลาง / ไม่มีจังหวะชีพจร / สัญญาณขาดช่วง) แทนข้อความรวม "สัญญาณอ่อนเกินไป" และแก้อาการอ่านค่า SpO2 ค้างไม่รู้จบ (FIFO เต็มแล้วตัวชี้อ่าน/เขียนเท่ากันจนดูเหมือนไม่มีข้อมูล)
...
|---|---|
| จำนวนกรณีทดสอบ (test cases) ทั้งหมด | **271** |
| ผ่าน | **271 (100%)** |
| ไม่ผ่าน | 0 |
| ความครอบคลุมของโค้ด (code coverage) โมดูลตรรกะ | **74%** (1,648 statements, พลาด 429) |
...
> รอบล่าสุดนับได้ **271 กรณี** ผ่านทั้งหมด (เพิ่มการวินิจฉัยสาเหตุของสัญญาณ SpO2 ที่คำนวณค่าไม่ได้
...
| **SpO2 (MAX30102): การหาค่านิ่ง + การวินิจฉัยสาเหตุที่สัญญาณใช้ไม่ได้** | `test_spo2_stability.py` | **36** |
| **SpO2: ไดรเวอร์ I2C (FIFO เต็ม/เซนเซอร์เงียบ/timeout)** | `test_max30102_driver.py` | **7** |
...
| `lib/spo2_max30102/max30102.py` | 117 | 15 | **87%** |
| `lib/spo2_max30102/spo2_monitor.py` | 221 | 31 | **86%** |
| `lib/ups.py` | 82 | 15 | **82%** |
| `lib/bp_monitor.py` | 290 | 65 | **78%** |
| `carekeeper_providers.py` | 455 | 114 | **75%** |
...
# แสดงรายชื่อกรณีทดสอบทั้ง 271 รายการ
